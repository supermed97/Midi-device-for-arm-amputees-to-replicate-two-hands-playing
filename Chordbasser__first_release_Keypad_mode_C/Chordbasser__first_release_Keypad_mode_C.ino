...
led(9,state);
 ...
 if (mySerial.available() > 0) {  
  ...
  while(a==144 ){
    mySerial.readBytes(temp,2);
    byte checkmult=checkinside(temp[0]);   
   // if( checkmult==1){
      chord[n]=temp[0];
    vel[n]=temp[1]; 
    n++;// } // n doesn't update continuosly when key is left pressed on,  meaning when key is being pressed on it comes out of the while loop 
          if (n>2 && state ==0){
...
if ((param[2]-param[1])==3){
  ...
  byte cast(char v){
    ...
    void multibass(byte *chordarray, byte velocity, byte command,byte sizeofarray){
      ...
      byte checkinside( byte skip){   
         for(int i=0; i<sizemode_c; i++){
          if(skip == mode_c_bass[i]){return 1;}
          }
        }
